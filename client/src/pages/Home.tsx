...
export default function Home() {
  ...
  const handleEarlyLeadSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!firstName.trim() || !email.trim()) return;

    setIsSubmitting(true);
    try {
      await fetch("/api/early-lead", {
        method: "POST",
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: 'var(--titan-font-base)' }}>
      ...
      <section className="py-20 px-6 text-center">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div id="hero-form" className="w-full max-w-2xl mx-auto">
            <form
              onSubmit={handleEarlyLeadSubmit}
            ...
            >
              <div className="flex flex-col sm:flex-row gap-4 mb-4">
                <input
                  ...
                  onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--titan-blue)'; }}
              ...
              <button
                ...
                style={{
                  padding: '16px 40px',
                  background: isSubmitting ? '#94A3B8' : 'var(--titan-grad-primary)',
                  color: '#FFFFFF',
                  border: 'none',
                  cursor: isSubmitting ? 'not-allowed' : 'pointer',
                }}
              >
                {isSubmitting ? 'Loading...' : 'Get My Free Roadmap'}
              </button>
            </form>
            <p
